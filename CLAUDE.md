...
- Features stay isolated — turning one flag off MUST NEVER disable another; shared handlers gate each feature on its own flag, verified by toggling each flag alone (G16)
- One canonical function per derived-and-stored field (e.g. display name), used by every writer and every comparator — two paths deriving it differently manufacture phantom diffs (G17)
- Bundling features in one PR is fine, but they must stay independent — a strict pre-merge review verifies each flag toggles alone without disabling another (G16, G18)
