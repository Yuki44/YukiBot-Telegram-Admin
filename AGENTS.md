...
| `trackNameChanges`       | SangMata-style name/@username change notices                                              | `nameChangeTracker` message handler · **rider:** the CSAM bio rotation reuses its `getChat` response to catch lurkers |
...
**Shared handlers (read before touching one):** `csamBioTrigger` fires when `csamDetection` **or** `languageDetection` is on and records recent messages for either — each branch gates on its own flag, so one being off never breaks the other (G16). The CSAM bio rotation (`scanner.ts`) calls `trackIdentity` from the same `getChat` response it already fetches, gated on `trackNameChanges`: with `csamDetection` off, `nameChangeTracker` still announces every change from messages — only the lurker coverage (users who never post) is lost, and disabling `trackNameChanges` cannot disable CP_ALERTA. When you edit a shared handler, re-verify every flag in its row.
