...
| `csamDetection`          | CSAM/impostor detection (**CP_ALERTA**) — bio + image                                     | **3 paths:** `csamBioTrigger` (urgent bio queue) · `csamImageScan` (image OCR) · rolling `scanner` (bio sweep). All three share one alert: `sendCsamAlert` claims `User.csamAlertedAt` per chat, so a post caught by the image tier *and* the bio scan raises **one** CP_ALERTA (an escalation to AUTO_BAN always alerts) |
...
| `trackNameChanges`       | SangMata-style name/@username change **notices**, per chat (each chat compares and announces against its own row; notices go to `logsTo`). The DB refresh itself is unflagged bookkeeping | `nameChangeTracker` message handler (always persists, announces on the flag) · **rider:** the CSAM bio rotation reuses its `getChat`/`getChatMember` responses, applying each read to every chat the user belongs to |
...
**Shared handlers (read before touching one):** `csamBioTrigger` fires when `csamDetection` **or** `languageDetection` is on and records recent messages for either — each branch gates on its own flag, so one being off never breaks the other (G16). The CSAM bio rotation (`scanner.ts`) feeds every identity it reads to `trackIdentityEverywhere`, which announces per chat on that chat's own `trackNameChanges`: with `csamDetection` off, `nameChangeTracker` still announces every change from messages, and disabling `trackNameChanges` cannot disable CP_ALERTA. Note that `getChat` only resolves peers the bot has seen recently (~88% of calls fail with `chat not found`), so the rotation gives **no** lurker coverage on its own; the coverage comes from the `getChatMember` presence probe the scanner fires after `CSAM_SCAN_MISS_LIMIT` consecutive `getChat` misses, which resolves any member and also prunes rows of users who left. When you edit a shared handler, re-verify every flag in its row.

**Modifiers (not features).** `Chat.features` also carries switches that shape one feature's output instead of enabling a capability of their own. They still default `false` (G8) and still toggle from `/togglefeature` and the Features screen, but G16 reads differently for them: a modifier must never disable the feature it belongs to, nor any other feature — it only changes where that feature's output goes. Turning `trackNameChanges` off makes its modifier moot by construction; that is not coupling between features.

| Modifier             | Belongs to         | Effect                                                                                      |
| -------------------- | ------------------ | ------------------------------------------------------------------------------------------- |
| `nameChangesVisible` | `trackNameChanges` | Off (default): notices go to `logsTo` only. On: also posted in the group. Never suppresses the tracking, the DB refresh, or the log-channel notice. |

**Identity is per-chat, never cross-chat.** Each chat compares an observation against its **own** `User` row and writes only that row (`confirmIdentity`), so one profile edit produces one notice per chat. A single *read*, however, is applied to every chat the user belongs to (`trackIdentityEverywhere`): name and @username are global on Telegram, so both the bio rotation **and** a message that reveals a change fan the same observation out — `nameChangeTracker` calls `trackIdentityEverywhere` whenever the message differs from what this chat stored, which is what stops a change seen in one group from never reaching the other. Only the avatar is shared (`syncPhotoAcrossChats`) — it is never announced. **The identity write belongs to `nameChangeTracker`, not to `trackUser`:** the membership middleware used to `$set` the name first, so the comparison right after it found nothing to announce — one lost change per user after every restart. `findOrCreate` now only `$setOnInsert`s identity, so no caller (including `resolveTarget`'s cached admin name) can overwrite the baseline; `handleJoin` also tracks identity, catching a rename that happened while the user was away. Names built from **invisible characters** (soft hyphens, Hangul fillers) are stripped for comparison and rendered as `(nombre invisible)`, and a dropped @handle is spelled out — otherwise both halves of the notice printed empty. A row carries `identityConfirmedAt`: until it is set, the stored name is an unverified leftover, so the first reading is adopted **silently**. That baseline is what keeps a newly-readable population (the `getChatMember` probe made ~82% of members legible for the first time) from announcing months of accumulated drift at once, as happened on 2026-08-02. Every announced change is also written to `ActivityLog` as `name_change`, so the dashboard keeps a queryable trail to compare against SangMata.
