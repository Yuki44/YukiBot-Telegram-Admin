...
| `csamDetection`          | CSAM/impostor detection (**CP_ALERTA**) — bio + image                                     | `csamBioTrigger` · `csamImageScan` · rolling `scanner` — three paths, one alert ([why](docs/agents/csam-detection.md)) |
...
| `trackNameChanges`       | SangMata-style name/@username change **notices**, per chat (notices go to `logsTo`); the DB refresh itself is unflagged bookkeeping | `nameChangeTracker` message handler · **rider:** the CSAM bio rotation reuses its `getChat`/`getChatMember` reads |
...
| `identityObservations`   | Diagnostics: records every identity reading (source + outcome) for 14 days, to explain a change that was never announced | `trackIdentity` (`observe`) — records only, never alters a notice; gated on its own flag (G16) |

**Shared handlers:** `csamBioTrigger` serves `csamDetection` **and** `languageDetection`; the CSAM bio rotation feeds `trackIdentityEverywhere`, which announces on each chat's own `trackNameChanges`. Each branch gates on its own flag (G16) — when you edit a shared handler, re-verify every flag in its row. Details: [docs/agents/csam-detection.md](docs/agents/csam-detection.md).

**Modifiers (not features).** `Chat.features` also carries switches that shape one feature's output instead of enabling a capability of their own. They still default `false` (G8) and toggle the same ways, but G16 reads differently: a modifier must never disable the feature it belongs to, nor any other — it only changes where that feature's output goes.
...
**Identity rules** — rationale, incidents and rendering details live in [docs/agents/name-tracking.md](docs/agents/name-tracking.md):

- Compare and write **per chat** (`confirmIdentity`); one read fans out to every chat the user is in (`trackIdentityEverywhere`).
- The identity write belongs to `nameChangeTracker`, never to `trackUser` — `findOrCreate` only `$setOnInsert`s identity.
- A row without `identityConfirmedAt` holds an unverified baseline: adopt the first reading **silently**.
- Every profile link goes through `profileHref` (`src/bot/helpers/html.ts`) — never build a `t.me` / `tg://` URL anywhere else (G17).
- Never link a **replaced** handle, and never leave one as bare `@handle` (Telegram auto-links it to its current owner).
- Announced changes are also written to `ActivityLog` as `name_change`.
...
| [docs/agents/name-tracking.md](docs/agents/name-tracking.md)                             | Identity/name-change tracking: rationale + incidents     |
| [docs/agents/csam-detection.md](docs/agents/csam-detection.md)                           | CSAM detection paths, alert dedup, presence-probe coverage |
