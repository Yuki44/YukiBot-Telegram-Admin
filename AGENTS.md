...
| `trackNameChanges`       | SangMata-style name/@username change notices, **per chat** (each chat compares and announces against its own row; notices go to `logsTo`) | `nameChangeTracker` message handler · **rider:** the CSAM bio rotation reuses its `getChat`/`getChatMember` responses |
...
**Identity is per-chat, never cross-chat.** Each chat compares an observation against its **own** `User` row and writes only that row (`confirmIdentity`), so one profile edit produces one notice per chat, when that chat sees it. Only the avatar is shared (`syncPhotoAcrossChats`) — it is never announced. A row carries `identityConfirmedAt`: until it is set, the stored name is an unverified leftover, so the first reading is adopted **silently**. That baseline is what keeps a newly-readable population (the `getChatMember` probe made ~82% of members legible for the first time) from announcing months of accumulated drift at once, as happened on 2026-08-02.
